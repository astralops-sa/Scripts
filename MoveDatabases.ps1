...
foreach ($path in @($DataPath, $LogPath)) {
    if (-not (Test-Path $path)) {
        ...
        try { New-Item -Path $path -ItemType Directory | Out-Null } 
        catch { Write-Info "Failed to create $path : $($_.Exception.Message)" "ERROR"; exit 1 }
    }
}
...
Write-Host "`n✅ Migration complete. Log saved to: $LogFile" -ForegroundColor Green
