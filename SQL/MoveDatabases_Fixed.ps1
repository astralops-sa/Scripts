...
try {
    ...
    if (-not (Get-Module -ListAvailable -Name SqlServer)) {
        Write-Warn "SqlServer module not found. Installing from PowerShell Gallery..."
        try {
            Install-Module -Name SqlServer -AllowClobber -Force -Scope CurrentUser -ErrorAction Stop
            Write-Ok "SqlServer module installed successfully."
        } catch {
    ...
    $confirmation = Read-Host "Please check Permissions and folders have been created correctly. Do you want to continue? (Y/N)"
    if ($confirmation -notin @("Y","y","Yes","yes")) {
        Write-Log "User cancelled operation before moving databases." "WARN"
        Write-Warn "Operation cancelled. No changes made."
        throw "Operation cancelled by user."
    }

    # Get user databases if not specified
    if (-not $DatabaseNames) {
        Write-Info "Fetching list of user databases..."
        Write-Log "Fetching list of user databases..." "INFO"
        ...
        try {
            ...
            Write-Err "Failed to query database list: $($_.Exception.Message)"
            Write-Log "Failed to query database list: $($_.Exception.Message)" "ERROR"
            exit 1
        }
    }
    ...
    foreach ($db in $DatabaseNames) {
        ...
        if ($db -in @('master','model','msdb','tempdb')) {
            Write-Warn "Skipping system database: $db"
            Write-Log "Skipping system database: $db" "WARN"
            continue
        }
        ...
        try {
            ...
            if (-not $files) {
                Write-Err "Database $db not found or inaccessible."
                Write-Log "Database $db not found or inaccessible." "ERROR"
                continue
            }
            ...
            Write-Err "Error moving $db : $($_.Exception.Message)"
            Write-Log "Error moving $db : $($_.Exception.Message)" "ERROR"
...
"@
    ...
    try {
        ...
        Write-Err "Failed to update instance-level default paths: $($_.Exception.Message)"
        Write-Log "Failed to update instance-level default paths: $($_.Exception.Message)" "ERROR"
    }
    ...
    try {
        ...
        Write-Err "Failed to restart SQL Server service: $($_.Exception.Message)"
        Write-Log "Failed to restart SQL Server service: $($_.Exception.Message)" "ERROR"
    }
