...
try {
    ...
    $SQLServiceAccount = ""
    try {
        $query = "SELECT ServiceName = servicename ,StartupType = startup_type_desc\t,ServiceStatus = status_desc,StartupTime = last_startup_time,ServiceAccount = service_account,IsIFIEnabled = instant_file_initialization_enabled FROM sys.dm_server_services WHERE servicename LIKE 'SQL Server%'"
    ...
    if ($confirmation -notin @("Y","y","Yes","yes")) {
        Write-Warn "User cancelled operation before moving databases."
        Write-Err "Operation cancelled. No changes made."
        throw
    }
